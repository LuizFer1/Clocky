...
if ($env:CLOCKY_VERSION) {
    $Tag = Normalize-Tag $env:CLOCKY_VERSION
} else {
...
$Version = Strip-V $Tag
$Asset = "clocky_${Version}_${Os}_${Arch}.zip"
$BaseUrl = "https://github.com/$Repo/releases/download/$Tag"
$AssetUrl = "$BaseUrl/$Asset"
$ChecksumsUrl = "$BaseUrl/checksums.txt"

$Tmp = Join-Path ([System.IO.Path]::GetTempPath()) ("clocky-install-" + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    $Binary = Get-ChildItem -Path $ExtractDir -Recurse -File -Filter 'clocky.exe' | Select-Object -First 1
    if (-not $Binary) {
        throw 'clocky.exe not found in archive'
    }

    New-Item -ItemType Directory -Force -Path $InstallDir | Out-Null
    $Dest = Join-Path $InstallDir 'clocky.exe'
    ...
    if ($pathAdded) {
        ...
        Write-Host 'Open a new terminal (or refresh PATH) if clocky is not found yet.'
    }

    Write-Host ""
    Write-Host "Installed clocky $Version to $Dest"
    & $Dest version
} finally {
