...
if [[ -n "${CLOCKY_VERSION:-}" ]]; then
  TAG="$(normalize_tag "${CLOCKY_VERSION}")"
else
...
VERSION="$(strip_v "$TAG")"
ASSET="clocky_${VERSION}_${OS}_${ARCH}.tar.gz"
...
BIN_SRC=""
if [[ -f "${TMPDIR}/clocky" ]]; then
  BIN_SRC="${TMPDIR}/clocky"
else
  BIN_SRC="$(find "$TMPDIR" -type f -name clocky | head -n 1 || true)"
fi
[[ -n "$BIN_SRC" && -f "$BIN_SRC" ]] || die "clocky binary not found in archive"

install -m 755 "$BIN_SRC" "${INSTALL_DIR}/clocky"
...
echo
echo "Installed clocky ${VERSION} to ${INSTALL_DIR}/clocky"
