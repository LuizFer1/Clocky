...
func TestExtractBinaryZip(t *testing.T) {
	payload := []byte("fake-clocky-binary")
	var buf bytes.Buffer
	zw := zip.NewWriter(&buf)
	w, err := zw.Create("subdir/clocky.exe")
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := ExtractBinary(buf.Bytes(), "clocky_1.0.0_windows_amd64.zip", dest); err != nil {
		t.Fatal(err)
	}
...
func TestExtractBinaryTarGz(t *testing.T) {
	payload := []byte("unix-clocky")
	var buf bytes.Buffer
	gw := gzip.NewWriter(&buf)
	tw := tar.NewWriter(gw)
	hdr := &tar.Header{
		Name: "clocky_1.0.0_linux_amd64/clocky",
		Mode: 0755,
	...
	dest := filepath.Join(t.TempDir(), "clocky")
	if err := ExtractBinary(buf.Bytes(), "clocky_1.0.0_linux_amd64.tar.gz", dest); err != nil {
		t.Fatal(err)
	}
	got, err := os.ReadFile(dest)
	if err != nil {
