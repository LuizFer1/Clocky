...
func extractZipBinary(archive []byte) ([]byte, error) {
	...
	for _, f := range zr.File {
		...
		if !isClockyBinaryName(base) || f.FileInfo().IsDir() {
			continue
		}
		rc, err := f.Open()
		if err != nil {
	...
	return nil, fmt.Errorf("clocky binary not found in zip archive")
}
...
func extractTarGzBinary(archive []byte) ([]byte, error) {
	...
	for {
		...
		if !isClockyBinaryName(base) {
			continue
		}
		return io.ReadAll(tr)
	}
	return nil, fmt.Errorf("clocky binary not found in tar.gz archive")
}

func isClockyBinaryName(base string) bool {
	return base == "clocky" || base == "clocky.exe"
}
